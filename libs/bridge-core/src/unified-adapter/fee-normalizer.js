...
class FeeNormalizer {
     ...
     */
    static normalizeRoutesFees(route) {
     ...
     */
    static calculateFeePercentage(inputAmount, fee) {
        try {
     ...
     */
    static convertPercentageToAmount(amount, percentage) {
        try {
     ...
     */
    static compareRoutesFees(route1, route2) {
        try {
     ...
     */
    static calculateEffectiveRate(inputAmount, outputAmount) {
        try {
            const input = BigInt(inputAmount);
     ...
     */
    static normalizeRoutesByFees(routes) {
     ...
     */
    static calculateAverageFee(routes) {
        if (routes.length === 0)
            return '0';
        let total = 0n;
     ...
     */
    static aggregateFeesAcquiredBridges(routes) {
     ...
     */
    static calculateFeeSavings(cheaperRoute, expensiveRoute) {
        try {
            const cheaper = BigInt(cheaperRoute.fee);
            const expensive = BigInt(expensiveRoute.fee);
            const absoluteSavings = expensive - cheaper;
            const percentageSavings = Number((absoluteSavings * 10000n) / expensive) / 100;
            return {
     ...
     */
    static estimateSlippage(inputAmount, outputAmount, marketRate) {
        try {
            const input = BigInt(inputAmount);
            const output = BigInt(outputAmount);
            if (input === 0n)
            ...
            if (marketRate) {
                ...
                const slippage = Number(((expectedOutput - output) * 10000n) / expectedOutput) / 100;
     ...
     */
    static groupRoutesByFeeRange(routes, rangeCount = 5) {
        if (routes.length === 0)
            return new Map();
        ...
        for (const route of routes) {
            ...
            if (rangeSize === 0n) {
                ...
                rangeIndex = Math.min(rangeCount - 1, Number((fee - minFee) / rangeSize));
            }
...
exports.FeeNormalizer = FeeNormalizer;
//# sourceMappingURL=fee-normalizer.js.map