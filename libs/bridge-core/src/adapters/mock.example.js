...
async function basicUsageExample() {
    ...
    const adapter = new mock_1.MockBridgeAdapter();
...
async function scenarioTestingExample() {
    ...
    for (const scenario of scenarios) {
        ...
        const adapter = (0, mock_1.createMockAdapter)(scenario);
        await adapter.initialize();
...
async function unitTestingPatternExample() {
    ...
    async function testBridgeService(adapter) {
        await adapter.initialize();
        const health = await adapter.getHealth();
        if (!health.healthy) {
    ...
    const happyAdapter = (0, mock_1.createMockAdapter)('happy-path');
    ...
    const unreliableAdapter = (0, mock_1.createMockAdapter)('unreliable');
    try {
