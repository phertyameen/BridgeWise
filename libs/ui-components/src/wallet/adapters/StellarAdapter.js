...
const SUPPORTED_CHAINS = ['stellar:public', 'stellar:testnet', 'stellar:futurenet'];
...
class StellarAdapter {
    constructor(options = {}) {
        this.id = 'stellar';
        this.name = 'Stellar Wallet';
        this.type = 'stellar';
        this.networkType = 'stellar';
        this.supportedChains = SUPPORTED_CHAINS;
        this.icon = 'https://stellar.org/favicon.ico';
        this.provider = null;
    ...
    get isAvailable() {
        if (typeof window === 'undefined')
            return false;
        const windowWithStellar = window;
    ...
    detectProvider() {
        ...
        const providers = ['freighter', 'rabet', 'albedo', 'xBull'];
     ...
     */
    getProviderByName(name, windowWithStellar) {
        const providerMap = {
            freighter: 'freighter',
            rabet: 'rabet',
            albedo: 'albedo',
    ...
    async connect(chainId) {
        ...
        if (!this.provider) {
            throw this.createError('WALLET_NOT_FOUND', 'No Stellar wallet found. Please install Freighter, Rabet, Albedo, or xBull.');
        }
        try {
    ...
    async getBalance(token) {
        ...
        try {
            ...
            if (assetCode && issuer) {
                ...
                const accountData = (await response.json());
                const assetBalance = accountData.balances.find((b) => b.asset_code === assetCode && b.asset_issuer === issuer);
...
//# sourceMappingURL=StellarAdapter.js.map