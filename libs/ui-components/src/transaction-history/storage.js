...
exports.TransactionHistoryStorage = void 0;
exports.createHttpTransactionHistoryBackend = createHttpTransactionHistoryBackend;
...
function fromStoredTransaction(transaction) {
    const now = new Date();
    ...
    return {
        ...
        slippagePercent: typeof transaction.slippagePercent === 'number' && Number.isFinite(transaction.slippagePercent)
            ? transaction.slippagePercent
            : 0,
        status: transaction.status === 'pending' || transaction.status === 'confirmed' || transaction.status === 'failed'
            ? transaction.status
            : 'pending',
...
exports.TransactionHistoryStorage = TransactionHistoryStorage;
function createHttpTransactionHistoryBackend(config) {
    ...
    return {
        ...
        getTransactionsByAccount: async (account) => {
            const response = await fetcher(`${config.baseUrl}/transactions/history?account=${encodeURIComponent(account)}`, {
                method: 'GET',
                headers: {
                    ...config.headers,
                },
